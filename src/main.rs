...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/proxy/search", post(handlers::handle_search_proxy))
...
mod handlers {
    ...
    #[derive(Deserialize)]
    pub struct ProxySearchReq {
        query: String,
        providers: Option<Vec<i64>>, // IDs of providers
        timeframe: Option<String>,
    }

    pub async fn handle_search_proxy(
        State(state): State<Arc<super::AppState>>,
        Json(req): Json<ProxySearchReq>,
    ) -> Json<Vec<SearchResult>> {
        // Get providers from the DB configuration
        ...
        let results = crate::search::perform_search(
            client, 
            providers_config, 
            req.query,
            req.timeframe
